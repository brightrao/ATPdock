...
public class PocSimSea {
	...
	private int end_ind;
	private String careLigTypes;  // "~ATP~~ADP~"
	...
	public PocSimSea(String poc_name, String q_seq, String q_apoc, 
			...
			int start_ind, int end_ind, String careLigTypes) throws Exception {
		...
		this.careLigTypes = careLigTypes;
	...
	private void searchBestTempl() throws Exception {
		...
		while (null != l) {
			...
			if (null != careLigTypes && !careLigTypes.equalsIgnoreCase("NULL") && !careLigTypes.contains("~NULL~") && !careLigTypes.contains("~"+ligtype+"~")) {
				l = br.readLine();
				count++;
				continue;
			}
...
//				e.printStackTrace();
	...
	public static double[] runAPoc(String templ_PocPath, String query_PocPath)throws Exception{
		...
		String lsalign_cmd = FixParams.APoc_exe + " " + templ_PocPath + " " + query_PocPath + " -fa 0 -plen 1";
		Process process;
		process = Runtime.getRuntime().exec(lsalign_cmd);
		StreamGobbler errorGobbler = new StreamGobbler(process.getErrorStream(), "Error");
		errorGobbler.start();
