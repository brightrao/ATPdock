...
public class NWAlign {
	...
	private String templseq;
	
	// output
	private int identicalLen;   	//
	private String queryAlignInfo; 	// ORIGINAL_START_POS#QUERY_ALIGNED_INFO
	private String matchAlignInfo; 	// MATCH_ALIGNED_INFO
	private String templAlignInfo; 	// ORIGINAL_START_POS#TEMPL_ALIGNED_INFO
	
	
	public NWAlign(String queryname, String queryseq, String templname, String templseq) {
		this.queryname = queryname;
		this.queryseq = queryseq;
		this.templname = templname;
		this.templseq = templseq;
				
		int gap_open=-11, gap_extn=-1;
		// input upper case sequences
		runNeedlemanWunsch(queryseq, templseq, gap_open,gap_extn);
	}
	 ...
	 * @return the templseq
	 */
	public String getTemplseq() {
		return templseq;
	}

	/**
	 * @return the identicalLen
	 */
	...
	/**
	 * @return the queryAlignInfo
	 */
	public String getQueryAlignInfo() {
		return queryAlignInfo;
	}

	/**
	 * @return the matchAlignInfo
	 */
	public String getMatchAlignInfo() {
		return matchAlignInfo;
	}

	/**
	 * @return the templAlignInfo
	 */
	public String getTemplAlignInfo() {
		return templAlignInfo;
	}

	
	public double getSeqIdenticalNorm8QueryOrigSeqLen(){
		return 1.0*this.identicalLen / queryseq.length();
	} 
	
	public double getSeqIdenticalNorm8TemplOrigSeqLen(){
		return 1.0*this.identicalLen / templseq.length();
	} 
	
	public double getSeqIdenticalNorm8MinOrigSeqLen(){
		int fm = templseq.length() > queryseq.length() ? queryseq.length() : templseq.length(); 
		return 1.0*this.identicalLen / fm;
	}
	
	public double getSeqIdenticalNorm8MaxSeqLen(){
		int fm = templseq.length() > queryseq.length() ? templseq.length() : queryseq.length(); 
		return 1.0*this.identicalLen / fm;
	}

	/**
	 * 	
	    String queryAlignSeq = "----ABC-D--E";
		String templAlignSeq = "A---E-G--G-E";
		
		tPos = getAlignedTemplPos8QueryPos(0); ans = 1;
		tPos = getAlignedTemplPos8QueryPos(1); ans = -1;
		tPos = getAlignedTemplPos8QueryPos(2); ans = 2;

	 * @param queryOriginalPos : the 'queryOriginalPos'-th amino acid in the original query sequence.
	 * @return
	   if the queryOriginalPos is not aligned templ pos return -1
	   @description obtain the index value in the original template sequence,
	    			which is aligned to the 'queryOriginalPos'-th amino acid in the original query sequence.
	 */
	public int getTemplPos8QueryPos(int _queryOriginalPos){
		int queryAlignStartPos = 0;
		String queryAlignSeq = queryAlignInfo;
		
		int templAlignStartPos = 0;
		String templAlignSeq = templAlignInfo;
		
		int ans = -1; 
		if (_queryOriginalPos >= queryAlignStartPos 
				&& (_queryOriginalPos-queryAlignStartPos)<queryAlignSeq.replaceAll("-", "").length()){
			int index = 0, k = queryAlignStartPos;
			while (k <= _queryOriginalPos){
				if ('-' != queryAlignSeq.charAt(index)){
					if (k == _queryOriginalPos){
						break;
					}
					k++;
				}
			...
			String cutTemplAlignSeq = templAlignSeq.substring(0, index+1);
			if (!cutTemplAlignSeq.endsWith("-"))
				ans = templAlignStartPos+cutTemplAlignSeq.replaceAll("-", "").length()-1;
		}
		
		return ans;
	}
	
	// the program is Needleman-Wunsch dynamic programming algorithm
	private void  runNeedlemanWunsch(String f1,String f2,int gap_open,int gap_extn){
...
//	    System.out.println();
	    ...
	    StringBuffer protAalignedInfo = new StringBuffer();
	    StringBuffer protBalignedInfo = new StringBuffer();
	    StringBuffer MatchalignedInfo = new StringBuffer();
...
//	    	System.out.print(sequenceA[i]);
	    	protAalignedInfo.append(sequenceA[i]);
	    }
//	    System.out.println();
	    for(i=1;i<=k;i++)
	    {
//	    	System.out.print(sequenceM[i]);
	    	MatchalignedInfo.append(sequenceM[i]);
	    }
//	    System.out.println();
	    for(i=1;i<=k;i++)
	    {
//	    	System.out.print(sequenceB[i]);
	    	protBalignedInfo.append(sequenceB[i]);
	    }
//	    System.out.println();	
//	    for(i=1;i<=k;i++)
//	    {
...
//	    System.out.println();
	    ...
	    this.queryAlignInfo = protAalignedInfo.toString();
	    this.templAlignInfo = protBalignedInfo.toString();
	    this.matchAlignInfo = MatchalignedInfo.toString();
	}
