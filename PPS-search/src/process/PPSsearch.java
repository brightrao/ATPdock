...
public class PPSsearch {
	...
	public static void main(String[] args) throws Exception{
		...
		seqidencut = args[4];
		jobnum = Integer.parseInt(args[5]);
		jobind = Integer.parseInt(args[6]);
		job_name = args[7];
	...
	private static void run_jobid_thSubJob() throws Exception {
		...
		PocSimSea pocsea = new PocSimSea("PDB52", protseq, apocpdb, seqid_cut, isPPSalign_or_APoc, REPORT_RECORD_NUM, start_ind, end_ind);
...
//				/****4****/similar_record_buf.append("</td>\n");
	...
	public static String runAPoc4Superpose(String aPocPath, String bPocPath, String save_sup)throws Exception{
		String lsalign_cmd = FixParams.APoc_exe + " " + aPocPath + " " + bPocPath + " -fa 0";
		Process process;
		process = Runtime.getRuntime().exec(lsalign_cmd);
		StreamGobbler errorGobbler = new StreamGobbler(process.getErrorStream(), "Error");
		errorGobbler.start();
